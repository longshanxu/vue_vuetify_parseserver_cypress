 ...
 * @LastEditTime: 2020-12-17 10:16:59
 * @LastEditors: Json.Xu
 * @Description: 
...
//https://vipc.cn/i/live/football/date/2020-12-17/prev
//https://vipc.cn/i/live/football/date/2020-12-17/next
...
async function GetTodayMoney() {
    try {
        ...
        const options = {
            url: 'https://vipc.cn/i/live/football/date/2020-12-17/prev',
            headers: {
                'User-Agent': 'request'
            },
            gzip: true
...
async function GetHistoryByID(matchId, datetemp, proxiedRequest) {
    try {
        ...
        proxiedRequest(options, async function (error, response, body) {
            if (!error && response.statusCode == 200) {
                let data = await JSON.parse(body);
