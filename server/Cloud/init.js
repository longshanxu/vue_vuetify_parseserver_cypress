 ...
 * @LastEditTime: 2023-04-14 13:52:41
...
//https://vipc.cn/i/live/football/date/2023-04-14/prev
//https://vipc.cn/i/live/football/date/2023-04-14/next
...
async function GetTodayMoney() {
  try {
    ...
    setTimeout(() => {
      // https://vipc.cn/i/live/football/date/today/next
      const options = {
        url: "https://vipc.cn/i/live/football/date/2023-04-14/prev",
        headers: {
          "User-Agent": "request",
        },
        gzip: true,
        ...
        if (!error && response.statusCode == 200) {
          ...
          for (let index = 0; index < data.items.length; index++) {
            ...
            for (let j = 0; j < element.matches.length; j++) {
              ...
              let Todaymoney = Parse.Object.extend("Money");
        ...
        if (!error && response.statusCode == 200) {
          ...
          if (data.odds && data.odds.length > 0) {
            ...
            for (let index = 0; index < data.odds.length; index++) {
              ...
              if (
                element.companyName == "平均欧赔" ||
                element.companyName == "竞彩" ||
                element.companyName == "威廉希尔" ||
                element.companyName == "Bet365" ||
                element.companyName == "10BET"
              ) {
...
Parse.Cloud.define("clearAllData", async (request) => {
  clearAllData();
  // GetTodayMoney();
});
...
//one by one
Parse.Cloud.define("OneByOne", async (request) => {
  OneByOne();
});
...
async function OneByOne() {
  ...
  const proxyHost = "tunnel.qg.net";
  const proxyPort = 10072;
  // 代理隧道验证信息
  const proxyUser = "CE834C03";
  const proxyPass = "09AE078E5194";
...
Parse.Cloud.define("GetDataByTen", async (request) => {
  GetDataByTen();
});
...
async function GetDataByTen() {
  ...
  for (let index = 0; index < items.length; index++) {
    ...
    setTimeout(() => {
      ...
      console.log("获取历史数据-------第" + index + "条数据，ip:" + ip);
    ...
    setTimeout(() => {
      ...
      console.log("获取盘口数据-------第" + index + "条数据:ip:" + ip);
    ...
    setTimeout(() => {
      ...
      console.log("获取赔率数据-------第" + index + "条数据,ip:" + ip);
