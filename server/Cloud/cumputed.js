 ...
 * @LastEditTime: 2020-06-29 19:48:55
 * @LastEditors: Json.Xu
 * @Description: 
...
Parse
    ...
    .define("cpu", async (request) => {
        ...
        for (let index = 0; index < items.length; index++) {
            ...
            if (results.get('weilian') == undefined || results.get('ticai') == undefined || results.get('bet10') == undefined) {
                continue;
            }
            ...
            console.log(element.get('league') + "----" + home + '  vs  ' + guest + "-----" + element.get('matchId'));
            ...
            weilianitem.returnRatio = math.format(weilianitem.returnRatio.replace('%', '') / 100, 3);

            //进行第二轮bet10的5%的浮动
            // let bet365item = results.get('bet365');

            // bet365item.returnRatio = math.format(bet365item.returnRatio.replace('%', '') / 100, 3);
           ...
           console.log('结局:'.red + finalitem );
            ...
            if (parseFloat(finalitem[1].replace('%', '')) >= 30 && parseFloat(finalitem[1].replace('%', '')) <= 40) {
                ...
                if(parseFloat(finalitem[0].replace('%', '')) <= -15) console.log("胜");
                if(parseFloat(finalitem[1].replace('%', '')) <= -15) console.log("平");
                if(parseFloat(finalitem[2].replace('%', '')) <= -15) console.log("负");
            }
            else {
