    ...
    if (pankoumoneyitem != undefined && pankoumoneyitem != null) {
      ...
      if (
        ...
        if (pankou2 > 0) {
          ...
          temparray[1] =
            parseInt(ouzhuanya[1].replace("%", "")) +
            parseInt(ouzhuanya[2].replace("%", ""));
...
Parse.Cloud.define("dodata", async () => {
  ...
  // queryMoneyResult.equalTo("date", "2023-06-06");
  ...
  for (var i = 0; i < MoneyResult.length; i++) {
    ...
    if(object.get("liangduibisai") && object.get("liangduibisai").length > 0 && object.get("qiushutouzhu")  && object.get("qiushutouzhu")[2].indexOf("no@two") == -1){
      ...
      AiData.set("sanhuWinXinli", object.get("sanhuxinli")[0]); //散户感觉主胜心理
      AiData.set("sanhuDrawXinli", object.get("sanhuxinli")[1]); //散户感觉平局心理
      AiData.set("sanhuLoseXinli", object.get("sanhuxinli")[2]); //散户感觉客胜心理
      AiData.set("zhuangjiaWinXinli", object.get("kaijuresult")[0]); //庄家开盘主胜概率
      AiData.set("zhuangjiaDrawXinli", object.get("kaijuresult")[1]); //庄家开盘平局概率
      AiData.set("zhuangjiaLoseXinli", object.get("kaijuresult")[2]); //庄家开盘客胜概率
      AiData.set("liangduiWinLishi", object.get("liangduilishi")[0]); //两队历史主队概率
      AiData.set("liangduiDrawLishi", object.get("liangduilishi")[1]); //两队历史平局概率
      AiData.set("liangduiLoseLishi", object.get("liangduilishi")[2]); //两队历史客队概率
