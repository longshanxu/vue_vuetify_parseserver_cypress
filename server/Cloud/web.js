 ...
 * @LastEditTime: 2023-08-11 15:47:50
...
Parse.Cloud.define("GetToday", async (request) => {
  try {
    if (
      request.params.date != "" &&
      ...
      for (let index = 0; index < results.length; index++) {
        ...
        const temp = {
          ...
          prediction: element.attributes.prediction, //预测结果
          qiuprediction: element.attributes.qiuprediction, //预测结果
        };
        data.push(temp);
      }
      return {
...
Parse.Cloud.define("AsyncData", async (request) => {
  try {
    //先清除数据。
    init.clearAllData();
    setTimeout(() => {
...
Parse.Cloud.define("AsyncGameData", async (request) => {
  try {
    init.GetTodayMoney();
...
Parse.Cloud.define("PythonRequest", async (request) => {
  try {
    console.log(request.params.data.length);
    if (request.params.data && request.params.data.length > 0) {
      var tempMoney = Parse.Object.extend("Money");
      var query = new Parse.Query(tempMoney);
      query.equalTo("date", "2023-08-11");
      query.limit(500);
      query.ascending("matchTime"); //matchTime,league
      const results = await query.find();
      for (let index = 0; index < results.length; index++) {
        const element = results[index];
        for (let index1 = 0; index1 < request.params.data.length; index1++) {
          const element1 = request.params.data[index1];
          if (element.get("matchId") == element1.matchId) {
             if (element1.prediction == 3) {
              element.set("prediction", "主胜");
            } else if (element1.prediction == 1) {
              element.set("prediction", "平局");
            }else if (element1.prediction == 0) {
              element.set("prediction", "客胜");
            }
            await element.save();
          }
        }
...
Parse.Cloud.define("PythonQiuRequest", async (request) => {
  try {
    console.log(request.params.data.length);
    if (request.params.data && request.params.data.length > 0) {
      var tempMoney = Parse.Object.extend("Money");
      ...
      for (let index = 0; index < results.length; index++) {
        ...
        for (let index1 = 0; index1 < request.params.data.length; index1++) {
          const element1 = request.params.data[index1];
          if (element.get("matchId") == element1.matchId) {
            if (element1.prediction == 0) {
              element.set("qiuprediction", "小球");
            } else if (element1.prediction == 1) {
              element.set("qiuprediction", "大球");
            } else if (element1.prediction == 2) {
              element.set("qiuprediction", "盘口球");
            }
            await element.save();
          }
        }
